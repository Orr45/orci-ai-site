...
export default function WeeklyCard({ item, onClick, index }: WeeklyCardProps) {
  ...
  return (
    <motion.article
      ...
      onMouseEnter={e => {
        (e.currentTarget as HTMLElement).style.boxShadow = '0 8px 40px rgba(0,209,255,0.15), 0 0 0 1px rgba(0,209,255,0.25)';
      }}
      onMouseLeave={e => {
    ...
    >
      ...
      <div className="relative w-full aspect-video overflow-hidden flex-shrink-0">
        {item.image ? (
          ...
          <div
            ...
            style={{ background: 'linear-gradient(135deg, rgba(0,209,255,0.08), rgba(168,85,247,0.08))' }}
          >
            {item.emoji}
          </div>
        )}
        ...
        {item.isNew && !item.isHot && (
          <div
            className="absolute top-3 left-3 flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold"
            style={{ background: 'rgba(0,209,255,0.85)', color: '#050d1a', backdropFilter: 'blur(8px)' }}
          >
            ✨ חדש
          </div>
        )}
      ...
      <div className="flex flex-col gap-3 p-4 flex-1">
        ...
        {/* Footer */}
        <div className="flex items-center justify-between pt-2 border-t" style={{ borderColor: 'rgba(0,209,255,0.08)' }}>
          <span className="text-xs" style={{ color: '#4a6a8a' }}>{item.source}</span>
          <span className="text-xs font-medium" style={{ color: '#00d1ff' }}>קרא עוד ›</span>
        </div>
      </div>
    </motion.article>
  );
